# -*- coding: utf-8 -*-
"""tx_月合約k_gspread__v03_1150301_完成_註解.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/10QxtUfy1I3U1t3JlVteBHFpvAV8f0w1J

# 程式碼總結說明：


此程式的主要目的是將台指期貨的每日交易數據（僅盤中 position 時段），
整理成「以合約為單位」的摘要資訊，包含：
 - 每個合約的開倉日期與開盤價
- 合約期間的最高價與最低價
- 合約的收盤日期與收盤價
...
# ============================================================
# 步驟 4：重新排列欄位順序
#   （因資料已只含盤中(position)，直接取用各欄位，無需跨時段彙總）
# ============================================================
df = df2[["date", "futures_id", "contract_date", "open", "max", "min", "close"]].copy()
dfa = df
...
# 合併三份資料為月K總表
mthA_Fx_contact_df = open_grouped     .merge(minmax_grouped, on="contract_date", how="left")     .merge(close_grouped, on="contract_date", how="left")
